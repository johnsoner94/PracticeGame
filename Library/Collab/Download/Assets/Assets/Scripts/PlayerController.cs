...
/* This script controls all player behavior and actions */
...
public class PlayerController : MonoBehaviour {
	...
	/// <Audio>
	public AudioClip clipJetPack;		// sound clip for doublejump
    public AudioClip clipKnock;			// sound clip for knockback
	private AudioSource audioJump;		// sound source for doublejump
	private AudioSource audioKnock;		// sound source for knockback


	// Adds a new AudioSource with the clip, loop boolean, playAwake, and volume set.
	...
	void Update () {
		...
		if (grounded) {							// if grounded...
			...
			audioJump.Stop();					// stops the audio jump sound effect so it is not playing when the player is grounded.
		} 
		...
		if (Input.GetButtonDown("Jump") && !doubleJumped && !grounded) {     // if up is pressed and player is not doubleJumped or grounded...		
			...
			audioJump.Play();			// plays the audio jump sound effect
		}
		...
		// if knockback is greater than 0...
			...
			audioKnock.Play();														// plays the audio knockback sound effect
            ...
            if(upShootDisabled == false) {
				...
				if ((Input.GetButton("Fire1"))) {
					...
					if (shotDelayCounter <= 0) {
						shotDelayCounter = shotDelay;                                           // adjust shotDelayCounter
						Instantiate (machineGunBullet, firePoint.position, firePoint.rotation); // make bullet and fire
					}
				}
